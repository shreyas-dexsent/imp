"""Tests for `test.mixed_parallel_test`."""
...
print("=" * 80)
print("MIXED PARALLEL VISION ENGINE TEST")
print("5 x BLOB_DETECTION + 5 x TEMPLATE_MATCHING = 10 CONCURRENT THREADS")
print("=" * 80)
...
def vision_thread(
    thread_id, module_name, camera_id="cam_webcam", fps_limit=1.0, duration=10
):
    """
    ...
    print(f"[Thread {thread_id}] Starting {module_name} with request_id: {request_id}")
    ...
    print(f"[Thread {thread_id}] ({module_name}) Sending STOP request")
    ...
    print(
        f"[Thread {thread_id}] ({module_name}) Completed - Received {thread_results[thread_id]['result_count']} results"
    )


# Configuration for mixed parallel tests
NUM_BLOB_THREADS = 5
NUM_TEMPLATE_THREADS = 5
TOTAL_THREADS = NUM_BLOB_THREADS + NUM_TEMPLATE_THREADS
TEST_DURATION = 10  # seconds
CAMERA_ID = "cam_webcam"
FPS_LIMIT = 1.0

print(f"\nStarting {TOTAL_THREADS} parallel threads...")
print(f"Configuration:")
print(f"  Camera: {CAMERA_ID}")
print(f"  Blob Detection Threads: {NUM_BLOB_THREADS}")
print(f"  Template Matching Threads: {NUM_TEMPLATE_THREADS}")
...
# Start blob_detection threads
print(f"Starting {NUM_BLOB_THREADS} blob_detection threads...")
for i in range(NUM_BLOB_THREADS):
    thread = threading.Thread(
        target=vision_thread,
        args=(thread_id, "blob_detection", CAMERA_ID, FPS_LIMIT, TEST_DURATION),
        daemon=True,
    )
...
# Start template_matching threads
print(f"Starting {NUM_TEMPLATE_THREADS} template_matching threads...")
for i in range(NUM_TEMPLATE_THREADS):
    thread = threading.Thread(
        target=vision_thread,
        args=(thread_id, "template_matching", CAMERA_ID, FPS_LIMIT, TEST_DURATION),
        daemon=True,
    )
    threads.append(thread)
    thread.start()
...
print(f"\nAll {TOTAL_THREADS} threads started!")
print("=" * 80)
...
expected_total = expected_results_per_thread * TOTAL_THREADS
print(
    f"\nExpected results (based on {FPS_LIMIT} FPS × {TEST_DURATION}s × {TOTAL_THREADS} threads): ~{expected_total:.0f}"
)
...
if all_process_times and total_results > 0:
    ...
    print(f"Per-thread throughput: {throughput/TOTAL_THREADS:.2f} results/second")
...
print("\n" + "=" * 80)
print("MIXED PARALLEL TEST COMPLETED")
print("=" * 80)
