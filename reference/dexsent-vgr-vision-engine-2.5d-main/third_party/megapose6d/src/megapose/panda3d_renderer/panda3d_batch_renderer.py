...
class Panda3dBatchRenderer:
    def __init__(
        ...
        self._direct_renderer: Optional[Panda3dSceneRenderer] = None
    ...
    def render(
        ...
        if self._n_workers == 0:
            assert self._direct_renderer is not None
            for n, scene_data_n in enumerate(scene_datas):
                is_valid = (
                    np.isfinite(scene_data_n.object_datas[0].TWO.toHomogeneousMatrix()).all()
                    and np.isfinite(scene_data_n.camera_data.TWC.toHomogeneousMatrix()).all()
                    and np.isfinite(scene_data_n.camera_data.K).all()
                )
                if is_valid:
                    renderings = self._direct_renderer.render_scene(
                        object_datas=scene_data_n.object_datas,
                        camera_datas=[scene_data_n.camera_data],
                        light_datas=scene_data_n.light_datas,
                    ...
                    h, w = scene_data_n.camera_data.resolution
                    renderings_ = CameraRenderingData(
                        rgb=np.zeros((h, w, 3), dtype=np.uint8),
                        normals=np.zeros((h, w, 1), dtype=np.uint8),
                        depth=np.zeros((h, w, 1), dtype=np.float32),
                ...
                list_rgbs[n] = torch.tensor(renderings_.rgb)
                if render_depth:
                    list_depths[n] = torch.tensor(renderings_.depth)
                if render_normals:
                    list_normals[n] = torch.tensor(renderings_.normals)
        else:
            for n, scene_data_n in enumerate(scene_datas):
        ...
        if self._device.type == "cuda":
            rgbs = rgbs.pin_memory().to(self._device, non_blocking=True)
        else:
        ...
        if render_depth:
            ...
            if self._device.type == "cuda":
                depths = depths.pin_memory().to(self._device, non_blocking=True)
            else:
                depths = depths.to(self._device)
        ...
        if render_normals:
            ...
            if self._device.type == "cuda":
                normals = normals.pin_memory().to(self._device, non_blocking=True)
            else:
                normals = normals.to(self._device)
    ...
    def _init_renderers(self, preload_cache: bool) -> None:
        ...
        if self._n_workers == 0:
            ...
            self._direct_renderer = Panda3dSceneRenderer(
