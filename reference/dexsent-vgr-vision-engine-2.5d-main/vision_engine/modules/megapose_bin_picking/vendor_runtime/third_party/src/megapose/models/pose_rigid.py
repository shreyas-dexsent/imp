...
class PosePredictor(nn.Module):
    ...
    def render_images_multiview(
        ...
        if random_ambient_light:
            ...
            # Use the same studio light rig for the RGB render even when
            # normals are requested. The normals pass is rendered separately
            # downstream, so forcing ambient-only here would flatten the actual
            # coarse-estimator RGB inputs into near-silhouettes.
