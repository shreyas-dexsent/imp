...
def _add_instance_id(
    detections: DetectionsType,
) -> DetectionsType:
    """Adds an instance_id column if it is missing."""
    if "instance_id" in detections.infos:
        return detections

    df = detections.infos.copy()

    def assign_group_ids(group: pd.DataFrame) -> pd.DataFrame:
        group = group.copy()
        group["instance_id"] = np.arange(len(group))
        return group

    df = df.groupby(["batch_im_id", "label"], group_keys=False).apply(assign_group_ids)
    detections.infos = df
    return detections
...
def _filter_detections(
    ...
    """Filters detections without importing the heavier inference utils module."""
...
class PoseEstimator(torch.nn.Module):
    ...
    def forward_coarse_model(
        ...
        SO3_grid = self._SO3_grid.to(observation.images.device)
        B = len(detections)
    ...
    def run_inference_pipeline(
        ...
        return_coarse_debug_data: bool = False,
    ) -> Tuple[PoseEstimatesType, dict]:
        ...
        if coarse_estimates is None:
            ...
            if detections is None and run_detector:
                ...
                detections = detections.to(observation.images.device)
                elapsed = time.time() - start_time
                timing_str += f"detection={elapsed:.2f}, "

            # Ensure that detections has the instance_id column
            assert detections is not None
            detections = _add_instance_id(detections)
            ...
            data_TCO_coarse, coarse_extra_data = self.forward_coarse_model(
                ...
                cuda_timer=cuda_timer,
                return_debug_data=return_coarse_debug_data,
            )
