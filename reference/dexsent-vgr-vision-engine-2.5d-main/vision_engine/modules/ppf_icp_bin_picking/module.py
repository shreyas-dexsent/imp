...
from vision_engine.modules.ppf_icp_bin_picking.runtime import (
    prewarm_ppf_icp_bin_picking,
    run_ppf_icp_bin_picking,
)
...
class _PpfIcpWorker:
    def __init__(self) -> None:
        ...
        self._thread = threading.Thread(
            target=self._run,
            daemon=True,
            name="ppf-icp-bin-picking-worker",
        )
        self._thread.start()
    ...
    def _run(self) -> None:
        while True:
            ...
            try:
                ...
                if kind == "prewarm":
                    task["result"] = prewarm_ppf_icp_bin_picking(
                        task.get("params") or {},
                    ...
                    task["result"] = run_ppf_icp_bin_picking(
                        bgr=task["bgr"],
                        depth=task["depth"],
                        params=run_params,
                        request_id=str(task.get("request_id") or ""),
...
class PpfIcpBinPickingModule(VisionModule):
    _worker_lock = threading.Lock()
    _prewarm_cache_lock = threading.Lock()
    _worker: _PpfIcpWorker | None = None
    ...
    @classmethod
    def _get_worker(cls) -> _PpfIcpWorker:
        with cls._worker_lock:
            if cls._worker is None or not cls._worker.is_alive:
                cls._worker = _PpfIcpWorker()
        ...
        *,
        ...
        with cls._prewarm_cache_lock:
            if (
                ...
                return cached
        result = worker.prewarm(params=params, request_id=request_id)
        with cls._prewarm_cache_lock:
            cls._last_prewarm_signature = signature
            cls._last_prewarm_result = dict(result or {})
        return result

    def _runtime_params_from_frame(self, frame_bundle: FrameBundle) -> tuple[Dict[str, Any], str]:
        ...
        return runtime_params, request_id
        ...
        try:
            if frame_bundle.rgb is None:
                return {
                    ...
                    "matches": [],
                    "terminal": True,
                    "error": "ppf_icp_rgb_missing",
                }
            runtime_params, request_id = self._runtime_params_from_frame(frame_bundle)
            ...
            if (
                ...
                and result.get("error") == "ppf_icp_no_candidate"
            ):
            ...
            if message.startswith("ppf_icp_"):
                error_code = message
            elif isinstance(exc, (ModuleNotFoundError, ImportError)):
                error_code = "ppf_icp_dependency_missing"
            else:
                error_code = "ppf_icp_inference_failed"
            result = {
                "valid": False,
                "matches": [],
                "terminal": True,
