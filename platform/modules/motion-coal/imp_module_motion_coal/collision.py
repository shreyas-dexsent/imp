...
and `Scene` from a `world.yaml` at `configure`, then on each `RobotState`
runs `algorithms.collision.is_in_collision` for that configuration and publishes
the contact count (0 = collision-free) as a `Scalar`.

This is the simplest Scene-consuming module; richer outputs (per-pair contacts,
clearances) and Scene-fill for live object poses follow the same shape.
"""
